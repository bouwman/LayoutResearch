    ...
    static func createActivitiesBasedOnLastActivityDate(lastDate: Date, activityNumber: Int?) -> [StudyActivity] {
        ...
        for i in 0..<Const.StudyParameters.searchActivityCount {
            ...
            if i <= number {
                ...
                activity = StudyActivity(startDate: Calendar.current.date(byAdding: .second, value: (i - number + extraDay) * 18, to: lastDate, wrappingComponents: false)!, number: i, type: .search)
            }
